...
ConfigureNetwork::ConfigureNetwork(const Core::System& system_, QWidget* parent)
    : QWidget(parent), ui(std::make_unique<Ui::ConfigureNetwork>()),
      lan_play_widget(new ConfigureLanPlay(this)), system{system_} {
    ui->setupUi(this);
    for (const auto& iface : Network::GetAvailableNetworkInterfaces())
        ui->network_interface->addItem(QString::fromStdString(iface.name));

    // LAN Play has its own entry in the menu bar and its settings live in a widget of their own;
    // this page shows that same widget so that everything about networking stays in one place.
    ui->verticalLayout_3->addWidget(lan_play_widget);
